...
/*
	...
	for(int i = 0; i < sampleCount; i++)
	{
		...
		float4 randomizedTexCoords = float4(splitInfo.TexCoords + float2(rotation.x * pk[i].x - rotation.y * pk[i].y, rotation.y * pk[i].x + rotation.x * pk[i].y)/750 * PoissonKernelScale[splitInfo.SplitIndex] * 0.5f, 0, 0);
	...
	return lerp(ShadowBrightness, 1.0, result / sampleCount * t);
}
...
PrepPSOutput PrepPassMainPS(PrepVertexShaderOutput input)
{
	...
	if(glowFactor > 0)
	{
		shadow = lerp(shadow, 1, glowFactor);
	}
	...
	//*********************************************************************************************
	output.ReflClr = 0;// input.Depth.x / input.Depth.y;
...
DataMaskPSOutput DataMaskMainPS(DataMaskVSOutput input)
{
	...
	//Next, Set the Depth Value
	//if(DoDepthMapping)
	output.Depth = DoDepthMapping ? input.Depth.x / input.Depth.y : 0;
	
	//Next, Set the Aux Depth Value
	output.AuxDepth = DoAuxDepth ? output.Depth : 0;
