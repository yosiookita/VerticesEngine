...
namespace VerticesEngine
{
	public class vxCamera2D : vxCamera
	{
		...
		public vxCamera2D(vxGameplaySceneBase sceneBase) : base(sceneBase)
        {
			...
			SimProjection = Matrix.CreateOrthographicOffCenter(0f, ConvertUnits.ToSimUnits(GraphicsDevice.Viewport.Width), ConvertUnits.ToSimUnits(GraphicsDevice.Viewport.Height), 0f, 0f, 1f);
        ...
        protected override void InitialiseRenderer()
        {
            ...
            Renderer.RenderingPasses.Add(new vxMainScene2DRenderPass(Renderer));
            Renderer.RenderingPasses.Add(new vxDistortionPostProcess2D(Renderer));
#if !__MOBILE__
		...
		public override void Update(GameTime gameTime)
		{
            ...
            float Width = (GraphicsDevice.Viewport.Bounds.Width - ViewportSafeAreaPadding) / this.Zoom;
            float Height = (GraphicsDevice.Viewport.Bounds.Height - ViewportSafeAreaPadding) / this.Zoom;
            ...
            _currentPosition += CameraTargetSpringStiffness * delta * inertia * (float)gameTime.ElapsedGameTime.TotalSeconds;
		...
		public Vector2 ConvertScreenToWorld(Vector2 location)
		{
			...
			t = GraphicsDevice.Viewport.Unproject(t, SimProjection, SimView, Matrix.Identity);
		...
		{
			Vector3 t = new Vector3(location, 0);
			t = GraphicsDevice.Viewport.Project(t, SimProjection, SimView, Matrix.Identity);
			return new Vector2(t.X, t.Y);
		}
	}
}