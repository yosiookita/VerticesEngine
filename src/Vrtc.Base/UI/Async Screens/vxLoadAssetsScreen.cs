...
namespace VerticesEngine.Screens.Async
{
    ...
    /// </summary>
    public class vxLoadAssetsScreen : vxBaseScene
    {
        #region Fields
        ...
        {
			//screenManager.InitMainRendertarget();
                                                       ...
                                                       bool coveredByOtherScreen)
        {
            ...
            if (otherScreensAreGone)
            {
                ...
                // Load the Games Global variables if needed
                if (vxEngine.Instance.Game.IsGameContentLoaded == false)
                {
                    vxEngine.Instance.Game.LoadGlobalContent();

                    // Set the "Content Loaded" flag
                    vxEngine.Instance.Game.IsGameContentLoaded = true;


                    // Load DLC content here
                    vxPluginManager.LoadContent();

                    Engine.Game.LoadPlayerProfile();
                }
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            if (loadingIsSlow)
            {
				...
				SpriteFont font = vxGUITheme.Fonts.Font;
                ...
                //Engine.vxGUITheme.vxLoadingScreen.PercentageComplete += 1;
                // Draw the text.
                SpriteBatch.Begin("Loading Assets Screen");

				if (Engine.SplashScreen != null && vxEngine.Instance.Game.IsGameContentLoaded == false) {
                    SpriteBatch.Draw (Engine.SplashScreen, viewport.Bounds, Color.White * TransitionAlpha);
				}

                SpriteBatch.DrawString(font, message, textPosition, Color.White);

                SpriteBatch.DrawString(vxGUITheme.Fonts.FontSmall, vxEngine.LoadingText, new Vector2(10), Color.Gray*0.25f);
				Engine.DrawVersionInfo(SpriteBatch, TransitionAlpha, Color.White);
