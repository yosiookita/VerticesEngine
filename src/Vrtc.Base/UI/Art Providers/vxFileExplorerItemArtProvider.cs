...
namespace VerticesEngine.UI.Themes
{
	...
	{
        public vxFileExplorerItemArtProvider(vxEngine Engine) : base(Engine)
		{
		...
		public virtual void Draw(object guiItem)
		{
			...
			Engine.SpriteBatch.Draw(DefaultTexture, item.Bounds, Theme.Border.Color);
			Engine.SpriteBatch.Draw(DefaultTexture, item.Bounds.GetBorder(-1), Theme.Background.Color * i);


			//Draw Icon
			if (item.ButtonImage != null)
			{
                Engine.SpriteBatch.Draw(item.ButtonImage, new Rectangle((int)(item.Position.X), (int)(item.Position.Y),
                                                                        16, 16), Color.White);
			}
			...
			//Draw Text String
            Engine.SpriteBatch.DrawString(Font, item.FileName,TextPos, Theme.Text.Color);

            if (item.IsDirectory == false)
            {
                Engine.SpriteBatch.DrawString(Font, item.LastUsed, TextPos + Vector2.UnitX * item.Width * 0.5f, Theme.Text.Color);

                Engine.SpriteBatch.DrawString(Font, item.FileSize, TextPos + Vector2.UnitX * item.Width * 3 / 4, Theme.Text.Color);
            }
		}
	}
}
