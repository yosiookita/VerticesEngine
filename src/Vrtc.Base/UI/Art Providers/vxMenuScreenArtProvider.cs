	...
	public class vxMenuScreenArtProvider : vxArtProviderBase, IGuiArtProvider
    {
       ...
       /// <summary>
		...
		public vxMenuScreenArtProvider(vxEngine Engine):base(Engine)
        {
			...
			//TitleBackground = Engine.InternalContentManager.Load<Texture2D>("Gui/DfltThm/vxGUITheme/vxMenuScreen/Bckgrnd_Nrml");
        }
		...
		{
			TitlePosition = new Vector2(Engine.GraphicsDevice.Viewport.Width / 2, 80) - vxGUITheme.Fonts.Font.MeasureString (MenuScreen.MenuTitle) / 2;
		}
		...
		public virtual void Draw(object guiItem)
        {
            ...
            if (IsTitleVisible)
            {
                ...
                TitleBoundingRectangle = new Rectangle(
                    ...
                    (int)(TitleFont.MeasureString(MenuScreen.MenuTitle).X * vxLayout.ScaleAvg + TitlePadding.X * 2),
                    (int)(TitleFont.MeasureString(MenuScreen.MenuTitle).Y * vxLayout.ScaleAvg + TitlePadding.Y * 2));

                if (IsTitleBackgroundVisible && MenuScreen.MenuTitle != "")
                {
                    Engine.SpriteBatch.Draw(vxGUITheme.SpriteSheet,
