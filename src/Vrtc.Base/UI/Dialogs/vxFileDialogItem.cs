...
namespace VerticesEngine.UI.Dialogs
{
	...
	public class vxFileDialogItem : vxScrollPanelItem
	{
		...
		public vxFileDialogItem(vxEngine Engine, FileInfo FileInfo) :
		base(Engine, FileInfo.Name, Vector2.Zero, null, 0)
		{
			Padding = new Vector2(4);

            this.FileInfo = FileInfo;
			//FileName = FilePath.GetFileNameFromPath();
			...
			ArtProvider = (vxFileDialoglItemArtProvider)vxGUITheme.ArtProviderForFileDialogItem.Clone();
		}
        ...
        public virtual void OnAsyncFileDetailsLoad(object sender, DoWorkEventArgs e)
        {
            ...
            string cachePath = Path.Combine(vxIO.PathToTempFolder, "cache", Path.GetFileNameWithoutExtension(FilePath));
            if (!Directory.Exists(cachePath))
            {
                // Decompress The Directory
                //vxIO.DecompressToDirectory(FilePath, vxIO.PathToTempFolder, null, false);
            ...
            ContentManager Content = new ContentManager(Engine.Game.Services, cachePath);
            ...
            */
            ...
            try
            {
                using (var fileStream = new FileStream(cachePath + "/img.png", FileMode.Open))
                {
                    ...
                    screenshot = Texture2D.FromStream(Engine.GraphicsDevice, fileStream);
