...
namespace VerticesEngine.UI.Controls
{
    ...
    /// </summary>
    public class vxScrollPanel : vxGUIControlBaseClass
    {
        /// <summary>
        /// The panel layout.
        /// </summary>
        ...
        public vxScrollPanel(vxEngine Engine, Vector2 Position, int Width, int Height) :
        base(Engine)
        {
            Alpha = 0.75f;
            ...
            ScrollBar = new vxScrollBar(Engine, this,
                new Vector2(
            ...
            ArtProvider = (vxScrollPanelArtProvider)vxGUITheme.ArtProviderForScrollPanel.Clone();
        }
        ...
        public virtual void AddItem(vxGUIControlBaseClass guiItem)
        {
            guiItem.DisableTouchSelectOnScroll = true;
            guiItem.ClickType = vxEnumClickType.OnRelease;


            //First, Check that the Width of the item is not wider than the panel it's self;
            /****************************************************************************************************/
            if (guiItem.Width > this.Bounds.Width)
            {
                guiItem.Width = this.Bounds.Width - (int)Padding.X * 3 - this.ScrollBarWidth;
            }
            ...
            if (AutoArrangeItems)
            {
                if (Items.Count > 0)
                {
                    ...
                    vxGUIControlBaseClass LastGuiItem = Items[Items.Count - 1];
                ...
                else
                {
                    ...
                    guiItem.Position = Padding;// new Vector2(Padding);
                }
                ...
                if (guiItem.Position.X + guiItem.Bounds.Width > Bounds.Width - Padding.X * 2 - ScrollBarWidth ||
            ...
            if (guiItem.GetBaseGuiType() == typeof(vxScrollPanelItem))
                ((vxScrollPanelItem)guiItem).Width = Bounds.Width - (int)Padding.X * 4 - ScrollBarWidth;
            ...
            guiItem.OnGUIManagerAdded(this.UIManager);
            ...
            for (int i = 0; i < Items.Count; i++)
            {
                ...
                if (item.IsVisible)
                {
                    ...
                    if (AutoArrangeItems)
                    {
                        ...
                        {
                            //If there's more than one item in teh scroll panel, get it.
                            vxGUIControlBaseClass LastGuiItem = Items[i - 1];
        ...
        public void SetScrollLength()
        {
            ...
            //Get The Max and Min Positions of Items to get the overall Height
            foreach (vxGUIControlBaseClass item in Items)
            {
                if (item.IsVisible)
                {
        ...
        public void AddRange(IEnumerable<vxGUIControlBaseClass> range)
        {
            foreach (vxGUIControlBaseClass item in range)
            {
                AddItem(item);
            }
        }
        ...
        public override void Update()
        {
            ...
            if (this.HasFocus && ScrollBar.HasFocus == false && ScrollBar.IsScrolling == false)
			{
				...
				else if (vxInput.IsMainInputDown())
				{
                    ...
                    ScrollBar.TravelPosition = InitialTravelPos -
                        ((vxInput.Cursor.Y - 
                          FirstTouchDownPosition.Y) / ((float)Height / (float)ScrollBar.ScrollBarHeight));

                    //FirstTouchDownPosition = vxInput.Cursor;

                    PanelSpeed = ScrollBar.TravelPosition - PreviousTravelPos;

                    if (Math.Abs(PanelSpeed) > 2)
                        DoSpeed = true;
                }
