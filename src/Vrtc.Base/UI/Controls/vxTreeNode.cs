...
namespace VerticesEngine.UI.Controls
{
	...
	public class vxTreeNode : vxButtonControl
	{
		...
		public vxTreeNode(vxEngine Engine, vxTreeControl TreeControl, string text, Texture2D Icon = null)
			: base(Engine, text, Vector2.Zero)
		{
			this.TreeControl = TreeControl;

			ArrowButton = new vxButtonImageControl(Engine, Vector2.Zero,
											vxInternalAssets.Textures.Texture_ArrowRight,
											vxInternalAssets.Textures.Texture_ArrowRight);
        ...
        void OnSelected(object sender, Events.vxGuiItemClickEventArgs e)
        {
            if (Entity != null)
		...
		{
            PlaySound(vxGUITheme.SoundEffects.MenuConfirm, 0.33f);
		...
		public override void Draw()
		{
			...
			Engine.SpriteBatch.Draw(DefaultTexture, Bounds, 
			...
			// Draw the Label
            //if(HasFocus == false)
			//Engine.SpriteBatch.DrawString(vxInternalAssets.Fonts.ViewerFont, Text, Position + LabelOffset+new Vector2(1), (HasFocus ? Color.Black * 0.5f : Color.Black));

            Engine.SpriteBatch.DrawString(vxInternalAssets.Fonts.ViewerFont, Text, Position + LabelOffset, this.ToggleState ? Color.Black : (HasFocus ? Color.Black : Color.White));

			// Finally, draw the Icon if it's present
			if(Icon != null)
				Engine.SpriteBatch.Draw(Icon, IconBounds, Color.White);
