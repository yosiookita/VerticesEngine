...
namespace VerticesEngine.Graphics
{
    ...
    {
        [vxGraphicalSettingsAttribute("ShadowQuality", "Shadow Quality", false, true, vxGameEnviromentType.ThreeDimensional)]
        ...
        public static int NumberOfShadowSplits
        {
            ...
            set { _numShadowSplits = MathHelper.Clamp(value, 1, 4); }
        }
        static int _numShadowSplits = 2;
        ...
        public static float ShadowBoundingBoxSize = 128;
        ...
        public vxCascadeShadowRenderPass(vxRenderer Renderer) :
        base(Renderer, "Cascade Shadow Pass", vxInternalAssets.Shaders.CascadeShadowShader)
        {
...
#if __ANDROID__
            ShadowMap = new RenderTarget2D(GraphicsDevice, ShadowMapSize * 2, ShadowMapSize * 2, false, SurfaceFormat.Color, DepthFormat.Depth24Stencil8);
            ShadowMapBlockTexture = new RenderTarget2D(GraphicsDevice, ShadowMapSize * 2, ShadowMapSize * 2, false, SurfaceFormat.Color, DepthFormat.Depth24);
#else
            ShadowMap = new RenderTarget2D(GraphicsDevice, ShadowMapSize * 2, ShadowMapSize * 2, false, SurfaceFormat.Single, DepthFormat.Depth24Stencil8);
            ShadowMapBlockTexture = new RenderTarget2D(GraphicsDevice, ShadowMapSize * 2, ShadowMapSize * 2, false, SurfaceFormat.Single, DepthFormat.Depth24);
        ...
        public void Prepare()
        {
            if (ShadowQaulity > vxEnumQuality.None)
            {
                ...
                //{
                Engine.GraphicsDevice.SetRenderTarget(ShadowMap);
                ...
                {
                    {
                        ...
                        {
                            GraphicsDevice.Viewport = ShadowViewports[i];
                        }
                    }
                    ...
                    for (int e = 0; e < Renderer.totalItemsToDraw; e++)
                    {
                        vxEntity3D entity = (vxEntity3D)Engine.CurrentScene.Entities[Renderer.drawList[e]];
                        if (entity.IsShadowCaster && entity.IsEntityDrawable)
                        {
                            for (int mi = 0; mi < entity.Model.Meshes.Count; mi++)
                            {
                                if (entity.Model.Meshes[mi].Material.IsShadowCaster)
                                {
                                    ShadowEffect.World.SetValue(entity.WorldTransform);
                                    entity.Model.Meshes[mi].Draw(ShadowEffect);
                                }
                            }
                        }
                    }
            ...
            for (int i = 0; i < Renderer.totalItemsToDraw; i++)
            {
                ...
                if (entity.IsEntityDrawable)
                {
                    ...
                    for (int mi = 0; mi < entity.Model.Meshes.Count; mi++)
                    {
                        ...
                        {
                            entity.Model.Meshes[mi].Material.UtilityEffect.CurrentTechnique = entity.Model.Meshes[mi].Material.UtilityEffect.Techniques["Technique_ShadowDebug"];
                            ...
                            if (vxCascadeShadowRenderPass.ShadowQaulity > vxEnumQuality.None)
                            {
                                ...
                                entity.Model.Meshes[mi].Material.UtilityEffect.ShadowMap = ShadowMap;
                                entity.Model.Meshes[mi].Material.UtilityEffect.ShadowTransform = ShadowSplitProjectionsWithTiling;
                                entity.Model.Meshes[mi].Material.UtilityEffect.TileBounds = ShadowSplitTileBounds;
                            }
        ...
        public void setShadowTransforms()
        {
            Matrix viewProj = Renderer.Camera.ViewProjection;
            Matrix viewProjInverse = Renderer.Camera.InverseViewProjection;
            Matrix projInverse = Renderer.Camera.InverseProjection;
            Matrix viewInverse = Renderer.Camera.InverseView;

            // figure out closest geometry empassing near and far plances based on the given scene bounding box
            var viewSpaceBB = vxGeometryHelper.TransformBoundingBox(shadowBounds, Renderer.Camera.View);
            ...
            {
                var d = Vector4.Transform(new Vector3(0, 0, c), Renderer.Camera.Projection);
            ...
            var splitData = Enumerable.Range(0, NumberOfShadowSplits).Select(i =>
            {
                ...
                var range = Math.Max(
                    1.0f / Renderer.Camera.Projection.M11 * -splitPlanes[i + 1] * 2.0f,
