...
{
    //Type t = typeof(Color); // MyClass is static class with static properties
    //            foreach (var p in t.GetFields(System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic))
    //            {
    ...
    public static Texture2D Resize(this Texture2D texture, vxEngine Engine, int NewWidth, int NewHeight)
    {
        //Create a New Render Target to take the resized texture.
        RenderTarget2D NewTexture = new RenderTarget2D(Engine.GraphicsDevice, NewWidth, NewHeight);

        //Set the New Render Target.
        Engine.GraphicsDevice.SetRenderTarget(NewTexture);
        Engine.GraphicsDevice.Clear(Color.Transparent);

        //Draw the original texture resized.
        Engine.SpriteBatch.Begin("Ext - Texture2D - Resize");
    ...
    public static Texture2D Crop(this Texture2D texture, vxEngine Engine, Rectangle CropRectangle)
    {
        //CropRectangle = new Rectangle(0, 0, texture.Width/2, texture.Height/2);
        //Create a New Render Target to take the resized texture.
        RenderTarget2D NewTexture = new RenderTarget2D(Engine.GraphicsDevice, CropRectangle.Width, CropRectangle.Height);
        ...
        //Draw the original texture resized.
        Engine.SpriteBatch.Begin("Ext - Texture2D - Crop");
        Engine.SpriteBatch.Draw(texture, Vector2.Zero, CropRectangle, Color.White);
        Engine.SpriteBatch.End();
