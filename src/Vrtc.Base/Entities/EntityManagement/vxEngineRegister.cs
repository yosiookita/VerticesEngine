...
namespace VerticesEngine
{
    ...
    public static class vxEntityRegister
    {
        ...
        internal static void RegisterAssemblyEntityTypes(vxIPlugin plugin)
        {
            ...
            //now load particles
            IEnumerable<Type> particleTypes = assembly.GetTypesWithAttribute(typeof(vxRegisterAsSandboxParticleAttribute));


            // Organise Sandbox Items into Categories and Sub Category Lists
            foreach (var type in particleTypes)
            {
                vxRegisterAsSandboxParticleAttribute attr = type.GetCustomAttribute<vxRegisterAsSandboxParticleAttribute>();

                var entityDef = new vxParticlePoolDefinition(type, attr.Name, attr.Category.ToString(), attr.PoolSize);

                // Particle Definition
                if (vxEntityRegister.ParticleDefinitions.ContainsKey(attr.Category.ToString()))
                {
                    vxConsole.WriteError(string.Format("ERROR LOADING PARTICLES: Particle '{0}' is using key '{1}' which already exists in the pool.", type.Name, attr.Category.ToString()));
                }
                else
                {
                    vxEntityRegister.ParticleDefinitions.Add(attr.Category.ToString(), entityDef);
                }
            }
        }
    }
