...
{
    [vxRegisterAsSandboxEntityAttribute("Water", "Engine Items", "Basic", "")]
            ...
            if (renderpass == vxRenderer.Passes.TransparencyPass)
            {
                ...
                foreach (vxModelMesh mesh in Model.Meshes)
                {
                 //   mesh.Material.IsDefferedRenderingEnabled = false;
                    ...
                    WorldTransform = Matrix.CreateScale(Scale / 250) * Matrix.CreateTranslation(Position);
                    ...
                    mesh.Material.TextureUVOffset += new Vector2(-0.0125f, 0.00751f);

                    mesh.Material.Shader.Parameters["CameraPos"].SetValue(Camera.Position);
