        ...
        private void DrawPointLight(vxCamera3D Camera)
		{
				...
				if (blendFactor > 0)
				{
					...
					foreach (ModelMesh mesh in vxInternalAssets.Models.PointLightSphere.Meshes)
					{
						...
						{
							Engine.GraphicsDevice.Indices = meshPart.IndexBuffer;
							Engine.GraphicsDevice.SetVertexBuffer(meshPart.VertexBuffer);

                        //Engine.GraphicsDevice.DrawIndexedPrimitives(PrimitiveType.TriangleList, 0, 0, meshPart.NumVertices, meshPart.StartIndex, meshPart.PrimitiveCount);
                        Engine.GraphicsDevice.DrawIndexedPrimitives(PrimitiveType.TriangleList, 0, meshPart.StartIndex, meshPart.PrimitiveCount);
                    }
					}
				}
